...
2026-07-20，公开 `v0.1.0` Release已在重装后的Debian 13 amd64 VPS完整走通普通用户流程：固定版本安装、自动诊断、客户端ZIP导出与SCP下载、Clash Verge/Hiddify导入以及REALITY/Hysteria2实际连接均通过。第一阶段在该证据边界内结论为 `PASS`。

Debian 12、Ubuntu 24.04、arm64、1C1G长时间稳定性和10 GB磁盘长期增长仍保留为后续验证项，不因本次单机成功升级为“已验证”。外部账户凭据轮换、云安全组和未启用防火墙的处置继续属于部署者运维责任。完整记录见[`v0.1.0 第一阶段实机验收报告`](../docs/PHASE1_ACCEPTANCE.md)。
...
8. public GitHub仓库 `filence/vpskit` 已完成首次提交并通过全部CI，首发版本采用 `v0.1.0`；`production-release` Environment、`v0.*` tag限制和新生产签名信任根已经建立，正式tag与草稿Release通过checksums、Ed25519清单、版本锁、SBOM、Linux权限和GitHub attestation复核后，用户明确决定直接公开，并以公开在线安装完成最后的普通用户从零部署与客户端回归；
