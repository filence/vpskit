...
- `01`–`06`拆分文档：可审阅、可比较差异的方案源文档；
- [VPSKit-完整方案-v1.1-20260717.md](VPSKit-完整方案-v1.1-20260717.md)：由拆分文档生成的单文件同步版。

重复的二进制方案ZIP不作为源码对象提交到GitHub。需要离线传递文档时，可在项目根目录使用PowerShell 7按需生成并校验本地ZIP：
...
生成的 `VPSKit-完整方案-v1.1-20260717.zip` 会保留在本地并被Git忽略。正式Release附件只承载安装、校验、签名、版本锁和SBOM资产，不混入重复的方案文档包。
...
本地源码快照位于项目根目录 `source-references/`。源码仓库不进入按需生成的方案ZIP，也不部署到VPS；该ZIP只包含文档和源码快照清单。
