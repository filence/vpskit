...
源码、生产签名信任根、受保护发布环境、tag和草稿资产核验均已完成。用户于2026-07-20明确决定直接公开 `v0.1.0`，把公开在线安装作为最后一次普通用户从零部署测试；下面两项干净VPS回归因此如实保留为发布后待验证，不改写为已完成。
...
`v0.1.0`已经公开，README的一键安装命令可供本次普通用户回归使用；完成上面两项VPS与客户端测试后，才能把首发实机发布回归表述为全部通过。
