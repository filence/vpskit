# VPSKit 功能演进完整方案 v0.3-R24

> 标题：VPSKit 功能演进完整方案
>
> 生成时间：2026-07-22 13:09
>
> 生成者：Codex
>
> 版本：v0.3-R24
...
- 原编制日期：2026-07-20
- 精简修订日期：2026-07-21
- 当前产品基线：VPSKit `v0.2.14-lab.1`；方案 A r0008、方案 B r0014、Salamander r0015、schema 9 白名单/自定义规则 r0012、`doctor --fix`、扩展 `system inspect`、Fail2ban SSH 白名单、规则刷新失败保护、Hysteria2 能力矩阵、性能基线、受管 UDP buffer、Adapter Registry 渐进接入与服务等待收口、只读带宽建议与系统更新候选检查已完成对应实机验收
...
不要在 schema 6 同时完成所有迁移。当前实际进度是：

- schema 10：已实现兼容投影，`instances[]` 使用 `id/protocol/adapter/enabled/listen.network/listen.port`；旧状态中仍保留协议专属字段，不删除凭据引用；
- `migrate check/plan/apply` 仍是唯一持久化迁移入口，schema 9→10 会创建可回滚状态事务；
- 后续才让 Adapter 接管渲染与变更分派，并在完整生命周期测试后考虑第三协议。
