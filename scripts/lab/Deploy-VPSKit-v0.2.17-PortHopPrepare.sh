...
install -d -m 0700 "$r";install -d -m 0700 "$(dirname "$b")";tar -xzf "$a" -C "$r";"$r/$v/vpskit" bundle verify --dir "$r/$v" >/dev/null;install -m 0700 /usr/local/bin/vpskit "$b";install -m 0755 "$r/$v/vpskit" /usr/local/bin/vpskit;installed=true;test "$(/usr/local/bin/vpskit version)" = "vpskit $v"
