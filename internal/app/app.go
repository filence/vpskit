...
func usageError() error {
	return errors.New("usage: vpskit <version|bundle verify|reality scan|cert status|cert renew|update self|update core|backup|restore <backup-id> --yes|rollback <transaction-id> --yes|recover|orphan scan|uninstall --yes|preflight|install balanced|instance <enable|disable|modify|delete>|node <show|modify>|rules <show|plan|apply>|hysteria2 inspect|migrate <check|plan|apply>|cleanup <plan|apply>|system <inspect|updates>|security fail2ban <status|plan|apply|remove>|support bundle|subscription <plan|configure|publish|status|rotate-read-token|revoke-read-token|rollback|remove>|menu|status|doctor|export>")
}
